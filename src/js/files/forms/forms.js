...
import { isMobile, _slideUp, _slideDown, _slideToggle, MTS } from "../functions.js";
...
export let formValidate = {
	...
	formClean(form) {
		...
		setTimeout(() => {
			...
			if (mtsModules.select) {
				let selects = form.querySelectorAll('.select');
				if (selects.length) {
					for (let index = 0; index < selects.length; index++) {
						const select = selects[index].querySelector('select');
						mtsModules.select.selectBuild(select);
					}
				}
			}
		}, 0);
	...
	function formSent(form, responseResult = ``) {
		...
		setTimeout(() => {
			if (mtsModules.popup) {
				...
				popup ? mtsModules.popup.open(popup) : null;
			}
		}, 0);
		// Очищаем форму
		formValidate.formClean(form);
